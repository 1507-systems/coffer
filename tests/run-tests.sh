#!/usr/bin/env bash
# run-tests.sh -- Simple test harness for lockbox
...
LOCKBOX_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
...
setup_test_env() {
    TEST_DIR=$(mktemp -d)
    export LOCKBOX_ROOT="${TEST_DIR}/lockbox"
    export LOCKBOX_VAULT="${LOCKBOX_ROOT}/vault"
    export LOCKBOX_CONFIG_DIR="${TEST_DIR}/config"
    export LOCKBOX_IDENTITY="${LOCKBOX_CONFIG_DIR}/identity.txt"
    export LOCKBOX_SOPS_CONFIG="${LOCKBOX_ROOT}/config/.sops.yaml"
    export LOCKBOX_SESSION_KEY="${LOCKBOX_CONFIG_DIR}/.session-key"

    mkdir -p "${LOCKBOX_ROOT}/lib" "${LOCKBOX_ROOT}/config" "${LOCKBOX_VAULT}" "${LOCKBOX_CONFIG_DIR}"

    # Copy lib files from the real project
    local real_root
    real_root="$(cd "${SCRIPT_DIR}/.." && pwd)"
    cp "${real_root}"/lib/*.sh "${LOCKBOX_ROOT}/lib/"
    cp "${real_root}"/config/*.yaml "${LOCKBOX_ROOT}/config/"
...
test_die_exits_nonzero() {
    ...
    local output
    output=$(LOCKBOX_NTFY_TOPIC="http://localhost:1/fake" bash -c '
        source "'"${LOCKBOX_ROOT}/lib/common.sh"'"
    ...
    assert_contains "$output" "lockbox: error: test error message" "die() should print error message"
}
...
test_warn_prints_to_stderr() {
    ...
    output=$(bash -c '
        ...
        warn "test warning"
    ' 2>&1)
    assert_contains "$output" "lockbox: warning: test warning" "warn() should print warning"
}
...
test_log_prints_to_stderr() {
    ...
    output=$(bash -c '
        ...
        log "test log"
    ' 2>&1)
    assert_contains "$output" "lockbox: test log" "log() should print message"
}
...
test_parse_path_valid() {
    ...
    output=$(bash -c '
        source "'"${LOCKBOX_ROOT}/lib/common.sh"'"
        export LOCKBOX_VAULT="'"${LOCKBOX_VAULT}"'"
        parse_path "cloudflare/dns-token"
        echo "cat=${LOCKBOX_CATEGORY} key=${LOCKBOX_KEY}"
    ' 2>&1)
...
test_list_with_test_vault() {
    ...
    cat > "${LOCKBOX_VAULT}/testcat.yaml" <<'YAML'
...
test_list_single_category() {
    cat > "${LOCKBOX_VAULT}/singlecat.yaml" <<'YAML'
...
test_list_missing_category_fails() {
    ...
    source "${LOCKBOX_ROOT}/lib/list.sh"
    local output
    output=$(LOCKBOX_NTFY_TOPIC="http://localhost:1/fake" cmd_list "nonexistent" 2>&1) || true
...
test_get_missing_category_fails() {
    ...
    touch "${LOCKBOX_IDENTITY}"
    chmod 600 "${LOCKBOX_IDENTITY}"
    export SOPS_AGE_KEY="AGE-SECRET-KEY-fake"

    local output
    output=$(LOCKBOX_NTFY_TOPIC="http://localhost:1/fake" cmd_get "nonexistent/key" 2>&1) || true
...
test_get_no_args_fails() {
    ...
    source "${LOCKBOX_ROOT}/lib/get.sh"
    local output
    output=$(LOCKBOX_NTFY_TOPIC="http://localhost:1/fake" cmd_get 2>&1) || true
    assert_contains "$output" "Usage" "get with no args should show usage"
}

# --- Entrypoint tests ---

test_lockbox_no_command_fails() {
    local real_root
    real_root="$(cd "${SCRIPT_DIR}/.." && pwd)"
    local output
    output=$(LOCKBOX_NTFY_TOPIC="http://localhost:1/fake" bash "${real_root}/bin/lockbox" 2>&1) || true
    assert_contains "$output" "No command specified" "lockbox with no command should fail"
}

test_lockbox_unknown_command_fails() {
    local real_root
    real_root="$(cd "${SCRIPT_DIR}/.." && pwd)"
    local output
    output=$(LOCKBOX_NTFY_TOPIC="http://localhost:1/fake" bash "${real_root}/bin/lockbox" frobnicate 2>&1) || true
    assert_contains "$output" "Unknown command" "lockbox with unknown command should fail"
}
...
test_lockbox_help() {
    ...
    real_root="$(cd "${SCRIPT_DIR}/.." && pwd)"
    local output
    output=$(bash "${real_root}/bin/lockbox" --help 2>&1)
    assert_contains "$output" "Usage: lockbox" "lockbox --help should show usage"
}

test_lockbox_version() {
    local real_root
    real_root="$(cd "${SCRIPT_DIR}/.." && pwd)"
    local output
    output=$(bash "${real_root}/bin/lockbox" --version 2>&1)
    assert_contains "$output" "lockbox 1.0.0" "lockbox --version should show version"
}

# --- Main ---

main() {
    echo "=== Lockbox Test Suite ==="
    echo ""
    ...
    # Entrypoint tests
    run_test test_lockbox_no_command_fails
    run_test test_lockbox_unknown_command_fails
    run_test test_lockbox_help
    run_test test_lockbox_version
